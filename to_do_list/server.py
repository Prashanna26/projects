...
class MyHttpRequestHandler(http.server.SimpleHTTPRequestHandler):
    def do_POST(self):
        if self.path == '/button-clicked':
            ...
            self.end_headers()
            self.get_html_element(text_area, status_msg , date_time)
    ...
    def get_html_element(self, data, status, date):
            task.append({'task': data , 'status': status, 'date_time':date})
    ...
    def send_to_html(self):       
        for index, tsk in enumerate(task):
            ...
            html_content = f'<div class="container {state_sts}">{tsk['task']}\
                <span class="date_cont">{dttm}</span>\
